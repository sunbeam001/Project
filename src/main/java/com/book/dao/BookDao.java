...
public class BookDao {
    ...
    private final static String MATCH_ALL_BOOK_SQL = "SELECT count(*) FROM book_info ";
    ...
    public int matchBook(String searchWord) {
        if (searchWord == null || searchWord.length() <= 0) {
            return jdbcTemplate.queryForObject(MATCH_ALL_BOOK_SQL, Integer.class);
        }
    ...
    public ArrayList<Book> queryBook(String sw) {
        boolean empty = null == sw || sw.trim().length() == 0;
        String swcx = empty ? "" : ("%" + sw.trim() + "%");
        final ArrayList<Book> books = new ArrayList<Book>();
        jdbcTemplate.query(empty ? QUERY_ALL_BOOKS_SQL : QUERY_BOOK_SQL, empty ? new Object[]{} : new Object[]{swcx, swcx}, new RowCallbackHandler() {
